...
### Sampling Textures
To display the sprite instead of a square, the shader will need a texture to display. While we could simply use a Texture 2D asset node to input the image that you chose earlier, this node will always provide the same texture. If we want our shader to work even if we change what image to display in the `Sprite Renderer`, our shader needs a property, through which the renderer will use to provide information to our shader.
